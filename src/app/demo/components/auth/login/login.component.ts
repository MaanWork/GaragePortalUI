  ...
  SurveyorId: any;
      ...
      this.loginService.onPostMethodSync(urlLink, ReqObj).subscribe(
        (data: any) => {
          if (data.Response) {
            ...
            this.mobileCodeList = obj.concat(data.Response); 
    ...
    submit(val) {
        if(this.password && this.username) {
            let urlLink =  `${this.CommonApiUrl}login/isValidUser`;
            let ReqObj = {
                ...
                (data: any) => {
                  ...
                  if (data.Response) {
                    ...
                    if (this.userType == 'Garage' || this.userType == 'Surveyor' || this.userType == 'Dealer' || this.userType=='Admin') {
                      ...
                      console.log('IIIIIIIIIIIIIIII',CompanyId);
                      sessionStorage.setItem('CurrencyidLogin',CompanyId);
                      ...
                      // let branchList: any[] = data?.Response?.LoginBranchDetails;
          ...
          if (data.Response) {
            if(data.AdditionalInfo){
              ...
              if(branchCode!='' && branchCode!='null' && branchCode!=null && branchCode!=undefined){
                Response['BranchCode'] = branchCode;
              }
             const Token = data?.Response?.Token;
              this.authService.login(data);
              this.authService.UserToken(Token);
              sessionStorage.setItem('UserToken',Token);
              if(data?.Response?.LoginBranchDetails){
                if(data?.Response?.LoginBranchDetails.length!=0){
                  data.Response['BranchCode'] = data?.Response?.LoginBranchDetails[0].BranchCode;
                  data.Response['BrokerBranchCode'] = data?.Response?.LoginBranchDetails[0].BrokerBranchCode;
                  data.Response['CurrencyId'] = data?.Response?.LoginBranchDetails[0].CurrencyId;
                }
              }
                ...
                if(details.PageType=='RP'){
                  sessionStorage.setItem('QuoteStatus','AdminRP');
                    ...
                    if(branchList.length != 0){
                      ...
                      userDetails.Response['ProductId'] = data.Response.BrokerCompanyProducts[0].ProductId;
                      userDetails.Response['ProductName'] = data.Response.BrokerCompanyProducts[0].ProductName;
                      ...
                      userDetails.Response['LoginType'] = 'B2CFlow';
    ...
    forgetSubmit(){
      ...
      this.loginService.onPostMethodBasicSync(urlLink, reqData).subscribe(
        (data: any) => {
          ...
          if (res?.ErrorMessage && res?.ErrorMessage.length > 0 || res?.Response?.ErrorMessage && res?.Response?.ErrorMessage.length > 0) {
    ...
    setTimeInterval() {
      ...
      onGuestLogin(data){
        ...
        const Token = data?.LoginResponse?.Response?.Token;
        this.authService.login(data.LoginResponse);
        this.authService.UserToken(Token);
        data.LoginResponse.Response['LoginType'] = 'B2CFlow';
        sessionStorage.setItem('Userdetails', JSON.stringify(data.LoginResponse));
        sessionStorage.setItem('UserToken', Token);
        sessionStorage.setItem('menuSection', 'navMenu');
        sessionStorage.removeItem('b2cType')
        let userDetails = JSON.parse(sessionStorage.getItem('Userdetails') as any);
        userDetails.Response['ProductId'] = '5'
        userDetails.Response['ProductName'] = 'Motor';
        userDetails.Response['BrokerBranchCode'] = data.LoginResponse?.Response?.LoginBranchDetails[0].BrokerBranchCode;
        userDetails.Response['BranchCode'] = this.branchValue;
        userDetails.Response['CurrencyId'] = this.userDetails.Response.CurrencyId;
        userDetails.Response['InsuranceId'] = this.insuranceId;
        sessionStorage.setItem('Userdetails', JSON.stringify(userDetails));
